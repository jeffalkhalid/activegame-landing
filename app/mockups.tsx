/* mockups.tsx — recréations fidèles des écrans PAG MATCH
   (Accueil, Les Défis, Le classement). Basé sur le code source RN.
   Composants purement présentationnels (aucun état). */
  ...
  return (
    <div className="sb" style={{ color: c }}>
      ...
      <div className="dots">
        ...
        <i style={{ border: `1px solid ${c}`, opacity: .9, borderRadius: 3, position: 'relative' }}><b style={{ position: 'absolute', inset: '1.5px', background: c, borderRadius: 1, display: 'block' }} /></i>
      </div>
    </div>
  );
}
...
/* ============== COMPAT RING ============== */
export function CompatRing({ score, color }: { score: number; color: string }) {
...
type Sugg = { n: string; av: string; avc: string; txt: string; lvl: string; score: number; tier: string; tlabel: string; league: string; lc: string; win: string; comp?: boolean };
  ...
  return (
    <div className="appscreen mm">
      ...
      <div className="mm-body">
        ...
        {sugg.map((p, i) => (
          <div className="sc" key={i} style={i === 0 ? { borderColor: 'rgba(16,185,129,.45)', boxShadow: '0 6px 16px rgba(16,185,129,.18)' } : undefined}>
            <div className="sc-bar" style={{ background: p.tier }} />
            <div className="sc-in">
...
type Pod = { rank: number; n: string; av: string; lvl: string; lc: string; medal: string; bg: string; bd: string; emoji: string; h: number; sz: number };
type Row = { r: number; n: string; av: string; avc: string; txt: string; league: string; lc: string; m: number; lvl: string };
      ...
      <div className="podium">
        {podium.map((p, i) => (
          <div className="pod" key={i}>
            <div className="pod-av" style={{ width: p.sz, height: p.sz, fontSize: p.sz * 0.32, background: '#FFC11A', color: '#0A0A0A', borderColor: p.bd, borderWidth: p.rank === 1 ? 3 : 2.5 }}>
              {p.av}
